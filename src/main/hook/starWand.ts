...
import type { FnTracePayload, NodeDebugger } from './socket'
...
export class StarWand {
	...
	constructor(
		public debugServer: NodeDebugger,
					...
					try {
						...
						applyRet =
							(() => {
								...
								// @ts-expect-error  忽略此处错误
								return this.config.eventInterceptors?.[`${key}:response`]?.({ applyRet, params: args })
							})() ?? applyRet
