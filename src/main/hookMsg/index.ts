...
function arkToText(msgList: MsgInfo[]) {
	for (const msgInfo of msgList) {
		for (const element of msgInfo.elements) {
			if (element.elementType !== 10 || !element.arkElement) continue
			const bytesData: {
				ver?: string
				prompt?: string
				config?: {
...
function msgFilter(msgList: MsgInfo[]) {
	...
	return msgList.filter((msgInfo) => {
		if (msgInfo.chatType !== 2) return true

		if (whitelist.includes(msgInfo.peerUid)) return true

		const botAttr = msgInfo.msgAttrs.get(22) as Record<string, unknown>

		if (blockRobot && botAttr && botAttr.botMetaData) return false

		if (msgInfo.elements.length === 1) {
			const element = msgInfo.elements[0]
			if (blockVideo && element?.elementType === ElementType.VideoElement) return false
			if (blockImage && element?.elementType === ElementType.PicElement) return false
			if (blockEmoji && element?.elementType === ElementType.FaceElement) return false
			if (
				blockSolitaire &&
				element?.elementType === ElementType.FaceElement &&
				element.faceElement?.chainCount !== null
			)
				return false
...
function msgListener(msgInfoList: MsgInfo[]) {
	const {
		messageMonitor: { keyword, favoriteList, whitelist },
	} = Utils.getConfig('main')

	for (const msgInfo of msgInfoList) {
		if (msgInfo.chatType !== 2) return
...
export const msgInterceptors: WrapperInterceptors = {
	'NodeIQQNTWrapperSession/getNTWrapperSession/getMsgService/addKernelMsgListener/onRecvMsg': ([msgInfoList]) => {
	...
	'NodeIQQNTWrapperSession/getNTWrapperSession/getMsgService/addKernelMsgListener/onMsgInfoListUpdate': ([
		msgInfoList,
	]) => {
		const msgInfo = msgInfoList[0]
		if (!msgInfo) return
	...
	'NodeIQQNTWrapperSession/getNTWrapperSession/getMsgService/getMsgsIncludeSelf:response': async ({ applyRet }) => {
	...
	'NodeIQQNTWrapperSession/getNTWrapperSession/getMsgService/getAioFirstViewLatestMsgs:response': async ({
		applyRet,
	}) => {
		const res = await applyRet
