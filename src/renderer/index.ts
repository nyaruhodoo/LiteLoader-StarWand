...
function summoningMagic() {
	setTimeout(() => {
		...
		const weather = document.querySelector(
			".user-profile-card__widgets .user-profile-card__weather",
		);
		const container = document.querySelector(
			".container-content",
		) as HTMLDivElement | null;
		if (!container || !globalContainer) return;
		...
		weather?.addEventListener(
			...
			(e) => {
				...
				// 加了皮肤后竟然是单独的容器....
				const miracleBackground = document.querySelector(
					".miracle-background",
				) as HTMLDivElement | null;
				if (miracleBackground) {
					miracleBackground.style.transition = "background 0.3s ease";
				}
