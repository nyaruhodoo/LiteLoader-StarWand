...
export class Utils {
   ...
   * 更新插件配置
   */
  static async updateConfig(config: ConfigType, mode: "renderer" | "main" | "preload") {
    const configUtils = PluginSettings[mode];
    configUtils.writeConfig(slug, config);
  }
  ...
  static mergeConfig(oldConfig: Record<string, any>, newConfig: ConfigType) {
    ...
    for (const [key, value] of Object.entries(oldConfig)) {
      ...
      if (
        ...
        Object.prototype.toString.call(value) !== Object.prototype.toString.call(targetObj[key])
      ) {
        continue;
      }
